...
// Types are now imported from StaffContext, so they are centralized
import { StaffMember, AttendanceRecordTypeFE, TemporaryExitTypeFE } from '../../../context/StaffContext';

import Card from '../../../components/ui/Card';
import Button from '../../../components/ui/Button';
import { format, startOfMonth, endOfMonth, eachDayOfInterval, isToday, isWeekend, differenceInMinutes, setHours, setMinutes, setSeconds, setMilliseconds, addMonths, subMonths, isEqual, startOfDay } from 'date-fns';

const REQUIRED_WORKING_MINUTES = 9 * 60;

// NEW: Detail Modal Component for historical records
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg p-6 w-full max-w-lg shadow-xl relative">
        ...
        <h3 className="text-xl font-semibold text-gray-800 mb-4">
          Attendance Details - {format(record.date, 'eeee, MMMM d, yyyy')}
        </h3>
        ...
        <div className="space-y-3 text-sm">
          <div className="flex justify-between items-center">
            ...
            <span className={`px-2 py-1 inline-flex leading-5 font-semibold rounded-full ${record.isWorkComplete ? 'bg-green-100 text-green-800' : 'bg-orange-100 text-orange-800'}`}>
              {record.status.charAt(0).toUpperCase() + record.status.slice(1).replace('_', ' ')}
            </span>
          </div>
          <div className="flex justify-between items-center">
            <span className="font-medium text-gray-600">Check In:</span>
            <span className="text-gray-800">{record.checkIn ? format(record.checkIn, 'HH:mm:ss') : 'N/A'}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="font-medium text-gray-600">Check Out:</span>
            <span className="text-gray-800">{record.checkOut ? format(record.checkOut, 'HH:mm:ss') : 'N/A'}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="font-medium text-gray-600">Total Working Time:</span>
            <span className="font-semibold text-gray-800">{formatDuration(record.totalWorkingMinutes)}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="font-medium text-gray-600">Required Time:</span>
            <span className={`font-semibold ${record.isWorkComplete ? 'text-green-600' : 'text-red-600'}`}>
              {formatDuration(REQUIRED_WORKING_MINUTES)} {record.isWorkComplete ? '(Completed)' : '(Incomplete)'}
            </span>
          </div>
                ...
                {record.temporaryExits.map(exit => (
                  <li key={exit.id} className="p-2 bg-gray-50 rounded-md">
                    <div className="flex justify-between items-center font-mono text-xs">
                      ...
                      <span className="font-sans font-semibold text-purple-600">({formatDuration(exit.durationMinutes)})</span>
                    </div>
...
const Attendance: React.FC = () => {
  ...
  const [selectedStaffIdForMonthly, setSelectedStaffIdForMonthly] = useState<string | null>(null);
  ...
  const [pendingCheckOutData, setPendingCheckOutData] = useState<{ attendanceId: string; staffName: string } | null>(null);
  ...
  const filteredStaff = activeStaffMembers.filter(staff =>
    staff.name.toLowerCase().includes(searchTerm.toLowerCase())
  );

  const calculateFrontendWorkingMinutes = useCallback((attendance: AttendanceRecordTypeFE): number => {
    let totalMinutes = 0;
    if (attendance.checkIn && attendance.checkOut) {
        return attendance.totalWorkingMinutes;
    } else if (attendance.checkIn && !attendance.checkOut) {
        totalMinutes = differenceInMinutes(new Date(), attendance.checkIn);
    }

    let tempExitDeduction = 0;
    (attendance.temporaryExits || []).forEach(exit => {
        if (!exit.isOngoing && exit.endTime) {
            tempExitDeduction += exit.durationMinutes;
        } else if (exit.isOngoing) {
            tempExitDeduction += differenceInMinutes(new Date(), exit.startTime);
        }
    });
    return Math.max(0, totalMinutes - tempExitDeduction);
  }, []);

  const handleCheckIn = async (staffId: string) => {
    try { await checkInStaff(staffId); }
    catch (err) { alert(`Check-in failed: ${err instanceof Error ? err.message : 'Unknown error'}`); }
  };

  const handleCheckOutAttempt = async (attendanceId: string, staffName: string) => {
    const attendance = attendanceRecordsFE.find(a => a.id === attendanceId);
    if (!attendance || attendance.checkOut) return;
    if (attendance.temporaryExits?.some(exit => exit.isOngoing)) {
      alert("Please end the ongoing temporary exit before checking out."); return;
    }
    const estimatedMinutes = attendance.checkOut ? attendance.totalWorkingMinutes : calculateFrontendWorkingMinutes(attendance);
    if (estimatedMinutes < REQUIRED_WORKING_MINUTES) {
      setPendingCheckOutData({ attendanceId, staffName });
      setShowConfirmModal(true);
    } else {
      await confirmCheckOut(attendanceId);
    }
  };

  const confirmCheckOut = async (attendanceId: string) => {
    try { await checkOutStaff(attendanceId); }
    catch (err) { alert(`Check-out failed: ${err instanceof Error ? err.message : 'Unknown error'}`); }
    finally { setPendingCheckOutData(null); setShowConfirmModal(false); }
  };

  const handleOpenTempExitModal = (attendanceId: string) => {
    const att = attendanceRecordsFE.find(a => a.id === attendanceId);
    if (!att || att.checkOut || (att.temporaryExits || []).some(e => e.isOngoing)) {
      alert("Cannot start temporary exit: Staff already checked out or an exit is ongoing."); return;
    }
    setSelectedAttendanceIdForTempExit(attendanceId);
  ...
  const handleSubmitTempExit = async () => {
    ...
    try {
      await startTemporaryExit(selectedAttendanceIdForTempExit, tempExitReason.trim());
    }
    catch (err) { alert(`Starting temp exit failed: ${err instanceof Error ? err.message : 'Unknown error'}`); }
    finally { setShowTempExitModal(false); setTempExitReason(''); setSelectedAttendanceIdForTempExit(null); }
  };

  const handleEndTempExit = async (attendanceId: string, tempExitId: string) => {
    try { await endTemporaryExit(attendanceId, tempExitId); }
    catch (err) { alert(`Ending temp exit failed: ${err instanceof Error ? err.message : 'Unknown error'}`); }
  };
  ...
  const getTodayAttendance = (staffIdToFind: string): AttendanceRecordTypeFE | undefined => {
    const todayStart = startOfDay(new Date());
    return attendanceRecordsFE.find(record =>
        record.staff.id === staffIdToFind &&
        isEqual(startOfDay(record.date), todayStart)
    );
  };

  const getMonthlyAttendanceIcon = (staffId: string, day: Date): React.ReactNode => {
    const dayStart = startOfDay(day);
    const record = attendanceRecordsFE.find(r =>
        r.staff.id === staffId &&
        isEqual(startOfDay(r.date), dayStart)
    );

    let icon: React.ReactNode = null; let title = "";
    if (!record) {
        if (isWeekend(day)) { title = "Weekend"; icon = <span className="block h-2 w-5 rounded-sm bg-gray-200" />; }
    ...
    switch (record.status) {
        case 'present':
            icon = <CheckCircle className={`h-5 w-5 ${record.isWorkComplete ? 'text-green-500' : 'text-orange-400'}`} />;
            break;
        case 'incomplete':
        ...
        case 'on_leave':
            ...
            break;
        default:
            icon = <span className="block h-2 w-2 rounded-full bg-gray-300" />;
    }
    return <div className="flex justify-center items-center h-full" title={title}>{icon}</div>;
  };
  ...
  const handleCalendarCellClick = (staffId: string, day: Date) => {
    ...
    if (record) {
        ...
        console.log(`No record found for staff ${staffId} on ${format(day, 'yyyy-MM-dd')}`);
    }
  };
        ...
        <>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Working Time</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Required</th>
                    <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Temp Exits</th>
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredStaff.map((staff) => {
                    ...
                    const workingMinutes = todayAttendance
                        ? (todayAttendance.checkOut ? todayAttendance.totalWorkingMinutes : calculateFrontendWorkingMinutes(todayAttendance))
                        : 0;
                    const remainingMinutes = Math.max(0, REQUIRED_WORKING_MINUTES - workingMinutes);
                    const ongoingTempExit = todayAttendance?.temporaryExits?.find(exit => exit.isOngoing);
                    return (
                      <tr key={staff.id} className="hover:bg-gray-50">
                        <td className="px-4 py-4 whitespace-nowrap">
                          <div className="flex items-center">
                            <img src={staff.image || '/placeholder-avatar.png'} alt={staff.name} className="h-10 w-10 rounded-full object-cover" />
                            <div className="ml-3"><div className="text-sm font-medium text-gray-900">{staff.name}</div><div className="text-sm text-gray-500">{staff.position}</div></div>
                          </div>
                        </td>
                        <td className="px-4 py-4 whitespace-nowrap">{todayAttendance ? <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${todayAttendance.status === 'present' && todayAttendance.isWorkComplete ? 'bg-green-100 text-green-800' : (todayAttendance.status === 'present' && !todayAttendance.isWorkComplete) || todayAttendance.status === 'incomplete' ? 'bg-orange-100 text-orange-800' : todayAttendance.status === 'late' ? 'bg-yellow-100 text-yellow-800' : todayAttendance.status === 'absent' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'}`}>{todayAttendance.status.charAt(0).toUpperCase() + todayAttendance.status.slice(1).replace('_', ' ')}{todayAttendance.status === 'present' && !todayAttendance.isWorkComplete && ' (Inc.)'}</span> : <span className="px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">Not Recorded</span>}</td>
                        ...
                        <td className="px-4 py-4 whitespace-nowrap"><span className={`text-sm ${remainingMinutes > 0 && workingMinutes > 0 && !todayAttendance?.isWorkComplete ? 'text-red-600' : (todayAttendance?.isWorkComplete ? 'text-green-600' : 'text-gray-700')}`}>{todayAttendance?.isWorkComplete ? 'Completed' : (remainingMinutes > 0 && workingMinutes > 0 ? `${formatDuration(remainingMinutes)} rem.` : formatDuration(REQUIRED_WORKING_MINUTES))}</span></td>
                        <td className="px-4 py-4 whitespace-nowrap">
                          {todayAttendance?.temporaryExits && todayAttendance.temporaryExits.length > 0 && (
                            <div className="space-y-1">
                              {todayAttendance.temporaryExits.map((exit) => (
                                <div key={exit.id} className="text-sm">
                                  <div className={`flex items-center space-x-1 ${exit.isOngoing ? 'text-blue-600 font-semibold' : 'text-gray-500'}`}>
                                    <span>{format(exit.startTime, 'HH:mm')} - {exit.endTime ? format(exit.endTime, 'HH:mm') : (exit.isOngoing ? 'Ongoing' : 'N/A')}</span>
                                    {!exit.isOngoing && exit.endTime && (
                                      <span className="text-purple-600">({formatDuration(exit.durationMinutes)})</span>
                                    )}
                                  </div>
                                  {exit.reason && <p className="text-xs text-gray-600 truncate max-w-[100px] hover:max-w-none hover:whitespace-normal" title={exit.reason}>{exit.reason}</p>}
                                </div>
                              ))}
                            </div>
                          )}
                  ...
                  {filteredStaff.length === 0 && (<tr><td colSpan={7} className="px-6 py-10 text-center text-gray-500">{searchTerm ? "No staff match search." : (activeStaffMembers.length === 0 ? 'No active staff.' : 'No results for today.')}</td></tr>)}
                </tbody>
              </table>
            </div>
          </Card>
          ...
          <div className="flex items-center justify-between mt-6 mb-2">
            ...
            <div className="flex items-center space-x-2">
                ...
                <span className="font-medium text-gray-700">{format(currentMonthDate, 'MMMM yyyy')}</span>
            ...
            <div className="overflow-x-auto pb-2">
              <table className="min-w-full border-collapse">
                <thead className="bg-gray-100">
                  <tr>
                    <th className="w-48 py-2 px-3 text-left text-xs font-medium text-gray-600 uppercase sticky left-0 bg-gray-100 z-10 border-r">Staff</th>
                    {daysInMonth.map((day) => (<th key={format(day, 'd')} className={`w-10 text-center py-2 text-xs font-medium uppercase border-b ${isWeekend(day) ? 'text-gray-500 bg-gray-50' : isToday(day) ? 'text-purple-700 font-bold bg-purple-50' : 'text-gray-500'}`}><div>{format(day, 'd')}</div><div className="text-[10px] font-normal">{format(day, 'EEE').charAt(0)}</div></th>))}
                  </tr>
                </thead>
                <tbody className="bg-white">
                  {filteredStaff.map((staff) => (
                    <tr key={staff.id} className="border-b border-gray-200 last:border-b-0 group">
                      <td className="py-2 px-3 sticky left-0 bg-white group-hover:bg-gray-50 z-10 border-r">
                        <div className="flex items-center">
                          <img src={staff.image || '/placeholder-avatar.png'} alt={staff.name} className="h-8 w-8 rounded-full object-cover mr-2"/>
                          <div>
                            <p className="text-sm font-medium text-gray-800 whitespace-nowrap">{staff.name}</p>
                          </div>
                        </div>
                      </td>
                      {daysInMonth.map((day) => (
                        ...
                        >
                          {getMonthlyAttendanceIcon(staff.id, day)}
                        </td>
                      ))}
                    </tr>
                  ))}
                  {filteredStaff.length === 0 && (<tr><td colSpan={daysInMonth.length + 1} className="py-8 text-center text-gray-500 border-t border-gray-200">No staff members match the current search.</td></tr>)}
                </tbody>
              </table>
            </div>
          </Card>
      ...
      {showTempExitModal && selectedAttendanceIdForTempExit && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-md shadow-xl">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">Record Temporary Exit</h3>
            <div className="space-y-4">
              ...
              <div className="flex justify-end space-x-3 pt-2">
                <Button variant="outline" onClick={() => { setShowTempExitModal(false); setTempExitReason(''); setSelectedAttendanceIdForTempExit(null); }}>Cancel</Button>
                <Button onClick={handleSubmitTempExit} disabled={!tempExitReason.trim()}>Start Exit</Button>
              </div>
            </div>
          </div>
        </div>
