...
export interface AdvancePaymentType {
  id: string;
  staffId: string | { id: string; name: string; image?: string; position?: string; };
  requestDate: string;
  amount: number;
  reason: string;
...
export interface NewAdvancePaymentPayload {
  staffId: string;
  amount: number;
  reason: string;
  repaymentPlan: string;
...
export interface NewPerformanceRecordPayload {
  staffId: string;
  month: string;
  year: number;
  rating: number;
...
export interface AttendanceRecordTypeAPI { _id: string; staffId: PopulatedStaffInfoAPI; date: string; checkIn: string | null; checkOut: string | null; status: 'present' | 'absent' | 'late' | 'incomplete' | 'on_leave'; temporaryExits: TemporaryExitTypeAPI[]; totalWorkingMinutes: number; isWorkComplete: boolean; notes?: string; createdAt?: string; updatedAt?: string; }
...
export interface AttendanceRecordTypeFE { id: string; staff: PopulatedStaffInfoFE; date: Date; checkIn: Date | null; checkOut: Date | null; status: 'present' | 'absent' | 'late' | 'incomplete' | 'on_leave'; temporaryExits: TemporaryExitTypeFE[]; totalWorkingMinutes: number; isWorkComplete: boolean; notes?: string; createdAt?: Date; updatedAt?: Date; }
...
export interface StaffContextType {
  ...
  addStaffMember: (staffData: NewStaffPayload) => Promise<StaffMember>;
  deleteStaffMember: (staffId: string) => Promise<void>;
  updateStaffMember: (staffId: string, updatedData: UpdateStaffPayload) => Promise<StaffMember>;
  getStaffById: (staffId: string) => StaffMember | undefined;

  positionOptions: PositionOption[];
  addPositionOption: (option: PositionOption) => void;

  advancePayments: AdvancePaymentType[];
  loadingAdvancePayments: boolean;
  errorAdvancePayments: string | null;
  fetchAdvancePayments: () => Promise<void>;
  requestAdvance: (payload: NewAdvancePaymentPayload) => Promise<void>;
  updateAdvanceStatus: (paymentId: string, status: 'approved' | 'rejected') => Promise<void>;

  attendanceRecordsFE: AttendanceRecordTypeFE[];
  ...
  performanceRecords: PerformanceRecordType[];
  loadingPerformance: boolean;
  errorPerformance: string | null;
  fetchPerformanceRecords: (filter: { month?: string; year?: number; staffId?: string }) => Promise<void>;
  recordPerformance: (payload: NewPerformanceRecordPayload) => Promise<void>;

  salaryRecords: SalaryRecordType[];
  ...
  fetchSalaryRecords: (filter?: {
    ...
    year?: number;
    month?: string;
    populateStaff?: 'true' | 'false' | boolean;
  }) => Promise<void>;
...
const initialPositionOptionsData: PositionOption[] = [
...
const mapApiAttendanceToFE = (apiRecord: AttendanceRecordTypeAPI): AttendanceRecordTypeFE => {
  ...
  return {
    id: apiRecord._id,
    staff: {
      id: apiRecord.staffId._id,
      name: apiRecord.staffId.name,
      image: apiRecord.staffId.image,
      position: apiRecord.staffId.position,
    },
    date: localDate,
    checkIn: apiRecord.checkIn ? parseISO(apiRecord.checkIn) : null,
    checkOut: apiRecord.checkOut ? parseISO(apiRecord.checkOut) : null,
    status: apiRecord.status,
    temporaryExits: (apiRecord.temporaryExits || []).map(apiExit => ({
      id: apiExit._id,
      attendanceId: apiExit.attendanceId,
      startTime: parseISO(apiExit.startTime),
      endTime: apiExit.endTime ? parseISO(apiExit.endTime) : null,
    ...
    createdAt: apiRecord.createdAt ? parseISO(apiRecord.createdAt) : undefined,
    updatedAt: apiRecord.updatedAt ? parseISO(apiRecord.updatedAt) : undefined,
  };
};

const mapApiPerformanceToFE = (apiRecord: PerformanceRecordAPIType): PerformanceRecordType => ({
    id: apiRecord._id,
    staffId: {
        id: apiRecord.staffId._id,
        name: apiRecord.staffId.name,
    ...
    metrics: apiRecord.metrics,
    createdAt: apiRecord.createdAt,
    updatedAt: apiRecord.updatedAt,
});

async function handleApiResponseError(response: Response, defaultErrorMessage: string): Promise<Error> {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      if (result && result.success === false && (result.error || result.message)) {
        errorMsg = result.error || result.message || defaultErrorMessage;
      }
    } catch (e) { /* Ignore if success response is not JSON or not relevant structure */ }
  }
...
export const StaffProvider: React.FC<StaffProviderProps> = ({ children }) => {
  ...
  const contextValue: StaffContextType = {
    staffMembers, loadingStaff, errorStaff, fetchStaffMembers,
    addStaffMember, deleteStaffMember, updateStaffMember, getStaffById,
    positionOptions, addPositionOption,
    advancePayments, loadingAdvancePayments, errorAdvancePayments, fetchAdvancePayments,
    requestAdvance, updateAdvanceStatus,
    attendanceRecordsFE, loadingAttendance, errorAttendance, fetchAttendanceRecords,
    checkInStaff, checkOutStaff, startTemporaryExit, endTemporaryExit,
    performanceRecords, loadingPerformance, errorPerformance, fetchPerformanceRecords, recordPerformance,
    salaryRecords, loadingSalary, errorSalary, fetchSalaryRecords,
    processSalary, markSalaryAsPaid,
  };
